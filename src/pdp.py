...
# BYOD monthly-CSV contract (optional column: days)
REQUIRED_PDP_COLUMNS: list[str] = ["well_id", "month", "oil_bbl"]
OPTIONAL_PDP_COLUMNS: list[str] = ["days"]
...
def template_csv() -> str:
    """A fill-in-and-upload template for the BYOD monthly schema."""
    return ("well_id,month,oil_bbl,days\n"
            "WELL-001,2023-01,9500,31\n"
            "WELL-001,2023-02,8800,28\n"
            "WELL-001,2023-03,8300,31\n")
...
def pv10(volumes_bbl, oil_price: float, loe_per_bbl: float = 12.0,
    ...
    monthly net revenue = oil_bbl x (oil_price - loe_per_bbl) x nri x (1 - severance)
    discounted end-of-month, effective-annual (econ_core.discounted_pv) — month 1
    is the first full month after the last history month.
    """
...
def screen_wells(df: pd.DataFrame, oil_price: float, loe_per_bbl: float = 12.0,
    ...
    r², current rate, remaining EUR, PV10 — sorted by PV10 descending, plus a list
    of (well_id, reason) for wells that could not be fit (short history etc.).
    """
    ...
    for well_id, g in df.groupby("well_id", sort=True):
        ...
        _, vols = forecast_volumes(fit, econ_limit_bopd, max_months)
        eur = float(vols.sum())
        pv = pv10(vols, oil_price, loe_per_bbl, nri, severance_frac, discount)
        rows.append({
            "well_id": str(well_id),
            "model": fit.model,
            ...
            "current_bopd": round(fit.current_rate_bopd, 1),
...
def deal_rollup(table: pd.DataFrame, asking_price_usd: float | None = None) -> dict:
    """Roll the per-well screen up to the deal: total PV10, total remaining EUR,
    total current production, and (when an asking price is given) $/flowing bbl
    and the PV10-vs-asking premium."""
    if table.empty:
        out = {"n_wells": 0, "total_pv10_usd": 0.0, "total_eur_bbl": 0.0,
               "total_current_bopd": 0.0}
    else:
        out = {
